...
if ($controllerChoice == 'login_user') {
...
else if ($controllerChoice == 'edit_account') {
    if (!isset($_SESSION['customer'])) {
        header("Location: ?controllerRequest=login_user");
        exit();
    }
...
else if ($controllerChoice == 'update_account') {
    if (!isset($_SESSION['customer'])) {
        header("Location: ?controllerRequest=login_user");
        exit();
    }
    ...
    $updatedCustomer = new Customer($customerId,$firstName,$lastName,$address,$city,$state,$postalCode,$emailAddress,
            $_SESSION['customer']->getPassword(),$_SESSION['customer']->getCustomerRoleTypeId(),
            $_SESSION['customer']->getDateAdded(),$_SESSION['customer']->getDateUpdated()
);

    $result = CustomerDB::updateCustomer($updatedCustomer);

    if ($result) {
        $_SESSION['customer'] = $updatedCustomer;
        $successMessage = "Your account has been updated successfully.";
        header("Location: ?controllerRequest=dashboard"); // <-- REDIRECT after update
        exit();
    } else {
        $errorMessage = "There was a problem updating your account.";
        include('edit_account.php');
    }
}
